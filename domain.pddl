...
;; un personaje importante se mueve a una localizaci√≥n adyacente que no sea peligrosa
...
;; un dragÛn se mueve con la princesa entre sus zarpas
...
;; un dragÛn secuestra a una princesa
	(:action secuestrar
		:parameters (?d ?p ?loc)
		:precondition (and
			(vivo ?d)
...
;; el villano se convierte en heroe
	(:action convertirseEnHeroe
		:parameters (?c ?locCab)
		:precondition (and
			(esCaballero ?c)
